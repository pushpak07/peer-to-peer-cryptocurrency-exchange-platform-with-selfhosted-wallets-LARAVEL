	...
	$.app.menu = {
		...
		manualScroller: {
			...
			updateHeight: function () {
				if (($body.data('menu') == 'vertical-menu' || $body.data('menu') == 'vertical-menu-modern' || $body.data('menu') == 'vertical-overlay-menu') && $('.main-menu')
					.hasClass('menu-fixed')) {
					$('.main-menu-content')
						.css('height', $(window).height() - $('.header-navbar').height() - $('.main-menu-header')
							.outerHeight() - $('.main-menu-footer').outerHeight());
					this.update();
				}
			}
		},
		...
		init: function (compactMenu) {
			if ($('.main-menu-content').length > 0) {
				var menuObj = this, defMenu = '';

				this.container = $('.main-menu-content');

				var state = sessionStorage.getItem('menu.collapsed');
				compactMenu = state !== null? JSON.parse(state): compactMenu;

				if (compactMenu) defMenu = 'collapsed';
					...
					.change(function () {
						...
						if (filter) {
							...
							if (searchFilter.parent().hasClass('has-sub')) {
								...
								searchFilter.show().parents('li').show().addClass('open').closest('li').children('a')
								            .show();
							}
						} else {
							// return to default
		...
		expand: function () {
			if (this.expanded === false) {
				...
				this.transit(function () {
					...
					if (($body.data('menu') == 'vertical-menu' || $body.data('menu') == 'vertical-menu-modern') && $('.main-menu')
						.hasClass('menu-fixed')) {
						$('.main-menu-content')
							.css('height', $(window).height() - $('.header-navbar').height() - $('.main-menu-header')
								.outerHeight() - $('.main-menu-footer').outerHeight());
		...
		saveToggleState: function(){
			sessionStorage.setItem('menu.collapsed', this.collapsed);
		},
