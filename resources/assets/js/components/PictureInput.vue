<template>
	<div ref="container" id="picture-input" class="picture-input">
		...
		<div v-else-if="supportsPreview">
			...
			<button v-if="imageSelected && !hideChangeButton" @click.prevent="selectImage" :class="buttonClass" v-html="strings.change"></button>
			<button v-if="imageSelected && removable" @click.prevent="removeImage" :class="removeButtonClass" v-html="strings.remove"></button>
			<button v-if="imageSelected && toggleAspectRatio && width !== height" @click.prevent="rotateImage" :class="aspectButtonClass" v-html="strings.aspect"></button>
		</div>
		<div v-else>
			<button v-if="!imageSelected" @click.prevent="selectImage" :class="buttonClass" v-html="strings.select"></button>
			<div v-else>
				<div v-html="strings.selected"></div>
				<button v-if="!hideChangeButton" @click.prevent="selectImage" :class="buttonClass" v-html="strings.change"></button>
				<button v-if="removable" @click.prevent="removeImage" :class="removeButtonClass" v-html="strings.remove"></button>
			</div>
		</div>
...
<script>
	export default {
		...
		props: {
			...
			buttonClass: {
				type: String,
				default: 'btn btn-primary round '
			},
			removeButtonClass: {
				type: String,
				default: 'btn btn-danger round'
			},
			aspectButtonClass: {
				type: String,
				default: 'btn btn-secondary round'
			},
			prefill: {
				type: [String, File],
				default: ''
...
<style scoped>
	...
	.picture-inner {
		...
		margin: 1em auto;
		padding: 1em;
		border: .2em dashed rgba(66,66,66,.15);
		border-radius: 8px;
