...
class ProcessDash implements ShouldQueue
{
    ...
    public function handle(DashAdapter $adapter)
    {
        ...
        $wallet = DashWallet::where('wallet_id', $this->data['wallet'])->first();
        ...
        if($tx['type'] == 'receive'){
            ...
            if ($transaction) {
                if ($confirmations >= $min_confirmations) {
                    if ($user = $wallet->user) {
                        $user->notify(new IncomingConfirmed('dash', $tx['value']));
                    }
                ...
                if ($confirmations < $min_confirmations) {
                    if ($user = $wallet->user) {
                        $user->notify(new IncomingUnconfirmed('dash', $tx['value']));
                    }
                }
            }
        }
